classdef GiniRandomForest < handle
    ...
    methods
        ...
        % Constructor
        function forest = GiniRandomForest(data, labels)
                forest.maxTrees = GiniRandomForest.defMaxTrees;
                forest.fracSize = GiniRandomTree.defFracSize;
                forest.targetLabel = GiniRandomForest.defTargetLabel;                
                forest.amount = 0;
                forest.data = data;
...
% End of get/set section
        ...
        function tree = getTree(forest, id)
            if (forest.amount <= 0)
                error(GiniRandomForest.error_empty);
            else if (id > forest.amount)
                    error(GiniRandomForest.error_outofbounds);
                else
                    i = 1;
                    tree = forest.first;
                    while (i < id)
        ...
        function addTree(forest, tree)
            if (isempty(forest.last))                
                ...
                if (forest.amount < forest.maxTrees)                    
                    ...
                    error(GiniRandomForest.error_max_size);
                end
            end
        end
        ...
        function train(forest)            
            for i=1:forest.maxTrees
                ...
                tree = GiniRandomTree(forest.data, forest.labels);
        ...
        function label = classifyv00(forest, x)
            if (isempty(forest.first))
                ...
                votesHist = GiniRandomForest.classHist(votes);
