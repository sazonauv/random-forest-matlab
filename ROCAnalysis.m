...
for r=1:repeat
    ...
    tree1 = ROCRandomTree(dataTrain, labelsTrain);
    tree1.fracSize = n;
    tree1.train();
    ROCs1{(r-1)*2+1} = tree1.generateROC(dataTest, labelsTest);
    
    % build a Randomized Tree
    tree2 = ROCRandomTree(dataTest, labelsTest);
    tree2.fracSize = n;
    tree2.train();
    ROCs1{(r-1)*2+2} = tree2.generateROC(dataTrain, labelsTrain);
...
%     tree1.fracSize = nmax;
%     tree1.train();
%     ROCs3{(r-1)*2+1} = tree1.generateROC(dataTest, labelsTest);
%     
%     % build a Randomized Tree
%     tree2 = ROCRandomTree(dataTest, labelsTest);
%     tree2.fracSize = nmax;
%     tree2.train();
%     ROCs3{(r-1)*2+2} = tree2.generateROC(dataTrain, labelsTrain);
    
end

% Plot ROC curve for a Randomized Tree
...
% % Plot ROC curve for a Randomized Tree
% [averROC3, deviations3] = ROCRandomTree.averageThresholdROC(ROCs3, 20);
% 
% 
% plot(averROC3(:,1), averROC3(:,2), '--k*','LineWidth',2,...
%                 'MarkerEdgeColor','k',...
%                 'MarkerFaceColor','k',...
%                 'MarkerSize',5)
% grid on
...
% hold all
% 
% errorbar(averROC3(:,1), averROC3(:,2), deviations3(:,1))
% hold all
% 
% herrorbar(averROC3(:,1), averROC3(:,2), deviations3(:,2))
% hold all
