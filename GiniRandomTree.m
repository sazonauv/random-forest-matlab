classdef GiniRandomTree < handle
    ...
    methods
        ...
        % Constructor
        function tree = GiniRandomTree(data, labels)
            tree.root = ROCTreeNode();
            tree.thrNum = GiniRandomTree.defThrNum;
            tree.leafSize = GiniRandomTree.defLeafSize;
            tree.minGain = GiniRandomTree.defMinGain*log2(max(labels)+1);
            tree.data = data;
            tree.labels = labels;
            tree.fracSize = GiniRandomTree.defFracSize; %ceil(log2(length(data(1,:))));
            tree.targetLabel = GiniRandomTree.defTargetLabel;            
        ...
        function test(tree, dataT, labelsT)
            len = length(labelsT);
            if (len == 0)
                error(GiniRandomTree.error_no_input);
            end
            err = 0;
            for i=1:len
                x = dataT(i,:);
    ...
    methods (Access = private)
        ...
        function label = applyRule(tree, node, x)
            ...
            if (isempty(node.left))
                if (node.label < 0)
                    error(GiniRandomTree.error_leaf_empty);
                else
                    label = node.label;
                end
                return;
        ...
        function infGain = infgain(tree, indexes, f, t, H)
            ...
            if (tree.lengthLeft > 1)
                Hl = tree.checkEntropy(GiniRandomTree.isLeft);
            else
                % Hl == 0; Hr == H;
                infGain = 0;
                return;
            ...
            if (tree.lengthRight > 1)
                Hr = tree.checkEntropy(GiniRandomTree.isRight);
            else
                % Hr == 0; Hl == H;
                infGain = 0;
                return;
        ...
        function e = checkEntropy(tree, splitFlag)
            ...
            e = 0;
            if (splitFlag == GiniRandomTree.isLeft)              
               for i=1:length(tree.classhist)
                    if (tree.classhist(i) > 0)
                        e = e - ( tree.classhist(i)/tree.lengthLeft )*( tree.classhist(i)/tree.lengthLeft - 1 );
                    end
                end
            else                
                for i=1:length(tree.classhist)
                    if (tree.classhist(i) > 0)
                        e = e - ( tree.classhist(i)/tree.lengthRight )*( tree.classhist(i)/tree.lengthRight - 1 );
                    end
                end
            end            
        end
        ...
        function e = entropy(tree, indexes)
            ...
            for i=1:length(h)
                if (h(i) > 0)
                    e = e - ( h(i)/length(indexes) )*( h(i)/length(indexes) - 1 );
                end
            end
        end
    ...
    methods (Static)
        ...
        function [averCurve, deviations] = averageThresholdROC(curves, samples)
            ...
            for s=1:samples
                ...
                for i=1:nrocs
                    curve = curves{i};
                    [npts ~] = size(curve);
                    [fpr, tpr] = GiniRandomTree.rocPointAtThr(curve, npts, T((s-1)*step + 1));
                    fprs(i) = fpr;
                    tprs(i) = tpr;
                end
        ...
        function printNodes(node)
            ...
            disp(str);
            GiniRandomTree.printNodes(node.left);
            GiniRandomTree.printNodes(node.right);
        end
