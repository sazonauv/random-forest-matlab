classdef ROCRandomTree < handle
    ...
    methods
        ...
        % Constructor
        function tree = ROCRandomTree(data, labels)
            tree.root = ROCTreeNode();
            tree.thrNum = ROCRandomTree.defThrNum;
            tree.leafSize = ROCRandomTree.defLeafSize;
            tree.minGain = ROCRandomTree.defMinGain*log2(max(labels)+1);
            tree.data = data;
            tree.labels = labels;
            tree.fracSize = ROCRandomTree.defFracSize; %ceil(log2(length(data(1,:))));
            tree.targetLabel = ROCRandomTree.defTargetLabel;            
    ...
    methods (Access = private)
        ...
        function label = applyRule(tree, node, x)
            ...
            if (isempty(node.left))
                if (node.label < 0)
                    error(ROCRandomTree.error_leaf_empty);
                else
                    label = node.label;
                end
                return;
        ...
        function infGain = infgain(tree, indexes, f, t, H)
            ...
            if (tree.lengthLeft > 1)
                Hl = tree.checkEntropy(ROCRandomTree.isLeft);
            else
                % Hl == 0; Hr == H;
                infGain = 0;
                return;
            ...
            if (tree.lengthRight > 1)
                Hr = tree.checkEntropy(ROCRandomTree.isRight);
            else
                % Hr == 0; Hl == H;
                infGain = 0;
                return;
        ...
        function e = checkEntropy(tree, splitFlag)
            ...
            e = 0;
            if (splitFlag == ROCRandomTree.isLeft)              
    ...
    methods (Static)
        ...
        function [averCurve, deviations] = averageThresholdROC(curves, samples)
            ...
            for s=1:samples
                ...
                for i=1:nrocs
                    curve = curves{i};
                    [npts ~] = size(curve);
                    [fpr, tpr] = ROCRandomTree.rocPointAtThr(curve, npts, T((s-1)*step + 1));
                    fprs(i) = fpr;
                    tprs(i) = tpr;
                end
        ...
        function printNodes(node)
            ...
            disp(str);
            ROCRandomTree.printNodes(node.left);
            ROCRandomTree.printNodes(node.right);
        end
